...
function StreamingMarkdown({ content }: { content: string }) {
  ...
  const tailComponents: any = useMemo(
    () => ({
      p: ({ children, node, ...props }: any) => <p {...props}>{animateText(children)}</p>,
      li: ({ children, node, ...props }: any) => <li {...props}>{animateText(children)}</li>,
      h1: ({ children, node, ...props }: any) => <h1 {...props}>{animateText(children)}</h1>,
      h2: ({ children, node, ...props }: any) => <h2 {...props}>{animateText(children)}</h2>,
      h3: ({ children, node, ...props }: any) => <h3 {...props}>{animateText(children)}</h3>,
      td: ({ children, node, ...props }: any) => <td {...props}>{animateText(children)}</td>,
    }),
    [],
  );
      ...
      {shouldCollapse && (
        <div className="flex justify-end mt-1">
          <button
            ...
            className="btn btn-xs rounded-full px-3 gap-1 opacity-70 active:opacity-100 transition-opacity"
            style={{ backgroundColor: "oklch(from oklch(var(--n)) calc(l - 0.05) c h)" }}
          >
            {expanded ? (
              <>
  ...
  return (
    <div className="mt-2">
      <button
        ...
        className="flex items-center gap-1.5 text-[10px] opacity-40 hover:opacity-60 transition-opacity"
      >
          ...
          {steps.map((step, i) => {
            ...
            return (
              <div key={i} className="flex items-start gap-1.5">
                {isThinking ? (
                  <Brain size={11} weight="duotone" className="shrink-0 mt-0.5" />
                ) : (
                  <Terminal size={11} weight="duotone" className="shrink-0 mt-0.5" />
                )}
                <span className={`${isThinking ? "line-clamp-3" : ""} break-all`}>
                  {content}
                </span>
              </div>
            );
...
export default function SessionPage() {
  ...
  return (
    ...
    >
      <div className={`navbar shrink-0 gap-1 transition-colors duration-300 ${isStreaming ? "bg-primary/15" : "bg-base-200"}`}>
        ...
        {messages?.map((message) => (
          <div
            ...
            className={`chat ${message.role === "user" ? "chat-end" : "chat-start"} ${
              initialMessageIdsRef.current && !initialMessageIdsRef.current.has(message._id)
                ? "chat-animate"
                : ""
            }`}
          >
                ...
                {message.streaming ? (
                  ...
                  <>
                    <CollapsibleMessage
                      ...
                      skipCollapse={witnessedStreamingRef.current.has(message._id)}
                    />
